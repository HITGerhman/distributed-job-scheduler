...
func (r *jobRepository) Create(ctx context.Context, job *domain.Job) (uint64, error) {
	const q = `
INSERT INTO jobs (
    name,
    cron_expr,
...
`
	res, err := r.exec.ExecContext(ctx, q,
		job.Name,
		job.CronExpr,
		job.Timezone,
		job.Payload,
...
func (r *jobRepository) GetByID(ctx context.Context, id uint64) (*domain.Job, error) {
	const q = `
SELECT
...
func (r *jobRepository) ListEnabled(ctx context.Context, limit int) ([]*domain.Job, error) {
	const qWithLimit = `
SELECT
    id, name, cron_expr, timezone, payload,
    timeout_seconds, max_retries, retry_backoff_seconds,
...
`
	...
	for rows.Next() {
		...
		if err := scanJob(rows, &job); err != nil {
			return nil, fmt.Errorf("scan enabled jobs failed: %w", err)
		}
...
`
	...
	for rows.Next() {
		var job domain.Job
		if err := scanJob(rows, &job); err != nil {
			return nil, fmt.Errorf("scan jobs failed: %w", err)
		}
		jobs = append(jobs, &job)
	}
	return jobs, rows.Err()
...
func (r *jobRepository) UpdateStatus(ctx context.Context, id uint64, fromStatus string, toStatus string) (bool, error) {
	const q = `
UPDATE jobs
